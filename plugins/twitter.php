        ...
        if(empty($twitterData) || isset($twitterData['error'])){
            ...
            foreach($twitterData as $item){
                    ...
                    if($update) {
                      ...
                      echo sprintf( __('%s', 'twitter-for-wordpress'),' <span class="twitter-timestamp"><abbr title="' . date(__('Y/m/d H:i:s'), $time) . '">' . $h_time . '</abbr></span><div class="twt-foot">' );
                     }         
 					?>
					<a href="http://twitter.com/intent/retweet?related=webdesignfs,thebenhunt&tweet_id=<?=$item['id_str']?>" class="twitter-retweet" title="Retweet" target="_blank">&bull; Retweet</a> 
                    <a href="http://twitter.com/intent/tweet?related=webdesignfs,thebenhunt&in_reply_to=<?=$item['id_str']?>" class="twitter-reply" title="Reply" target="_blank">&bull; Reply</a> 
                    <a href="http://twitter.com/intent/favorite?related=webdesignfs,thebenhunt&tweet_id=<?=$item['id_str']?>" class="twitter-favourite" title="Favourite" target="_blank">&bull; Favourite</a>
                   <?php echo '</div></p>';
